...
$link_res = "http://gunlacviet.vn/res/"; # link resource cua zing (hoac cua mot site nao do)
$folder_save = "newflash/"; # thu muc chua flash moi khi he thong tu tai ve
$folder_flash = "flashSV1/"; # thu muc chua flash cu~ (bat cu flash cua phien ban nao cung duoc)

$language = "vietnam/"; // ngon ngu file swf (neu rip cua zing thi cu de vietnam. cua nuoc khac thi thay vo cho phu hop)
...
if(count($file_1) > 0) {
	...
	foreach ($file_1 as $fileDown) {
		...
		$save = saveFile($folder_flash.$fileDown, $folder_save.$fileDown);
		if($save) {
...
if(count($file_2) > 0) {
	...
	foreach ($file_2 as $fileDown) {
		# duyet tat ca cac file roi save lai vao thu muc flash moi
		$save = saveFile($folder_flash.$folder_flash_ui.$fileDown, $folder_save.$folder_flash_ui.$fileDown);
		if($save) {
...
if(count($file_3) > 0) {
	...
	foreach ($file_3 as $fileDown) {
		# duyet tat ca cac file roi save lai vao thu muc flash moi
		$save = saveFile($folder_flash.$folder_flash_swf.$fileDown, $folder_save.$folder_flash_swf.$fileDown);
		if($save) {
...
if(count($file_4) > 0) {
	...
	foreach ($file_4 as $fileDown) {
		# duyet tat ca cac file roi save lai vao thu muc flash moi
		$save = saveFile($folder_flash.$folder_flash_xml.$fileDown, $folder_save.$folder_flash_xml.$fileDown);
		if($save) {
